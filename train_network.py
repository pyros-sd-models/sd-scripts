...
class NetworkTrainer:
    ...
    def train(self, args):
        with Live(monitor_system=True, report="html", dvcyaml="dvc.yaml") as live:
            # Convert args to a dictionary and filter serializable items
            serializable_args = {
                key: str(value)
            ...
            if support_multiple_lrs:
                ...
                if (
                    ...
                    text_encoder_lr = (
                        None
                        if len(args.text_encoder_lr) == 0
                        else args.text_encoder_lr[0]
                    )
            try:
                if support_multiple_lrs:
            ...
            optimizer_train_fn, optimizer_eval_fn = (
                train_util.get_optimizer_train_eval_fn(optimizer, args)
            )
            ...
            if args.fp8_base or args.fp8_base_unet:
                ...
                )  # this seems to be safer than above
            ...
            if args.deepspeed:
                ...
                ds_model = deepspeed_utils.prepare_deepspeed_model(
                    ...
                    text_encoder2=(text_encoders[1] if flags[1] else None)
            ...
            def save_model_hook(models, weights, output_dir):
                ...
                if accelerator.is_main_process or args.deepspeed:
                    ...
                    for i, model in enumerate(models):
                        if not isinstance(
                            model, type(accelerator.unwrap_model(network))
                        ):
                ...
                for dataset in train_dataset_group.datasets:
                    ...
                    for subset in dataset.subsets:
                        ...
                        if is_dreambooth_dataset:
                            ...
                            if subset.is_reg:
                                image_dir_or_metadata_file = (
                                    None  # not merging reg dataset
                                )
                        else:
            ...
            if accelerator.is_main_process:
                ...
                live.log_params(serializable_args)
            ...
            for epoch in range(epoch_to_start, num_train_epochs):
                ...
                for step, batch in enumerate(skipped_dataloader or train_dataloader):
                    ...
                    with accelerator.accumulate(training_model):
                        ...
                        if network_has_multiplier:
                            ...
                            accelerator.unwrap_model(network).set_multiplier(
                                multipliers
                            )

                        text_encoder_conds = []
                        text_encoder_outputs_list = batch.get(
                            "text_encoder_outputs_list", None
                        )
                            ...
                            with torch.set_grad_enabled(
                                ...
                                if args.weighted_captions:
                                    ...
                                    if args.full_fp16:
                                        encoded_text_encoder_conds = [
                                            c.to(weight_dtype)
                                            for c in encoded_text_encoder_conds
                                        ]
                            ...
                            if len(text_encoder_conds) == 0:
                                ...
                                for i in range(len(encoded_text_encoder_conds)):
                                    if encoded_text_encoder_conds[i] is not None:
                                        text_encoder_conds[i] = (
                                            encoded_text_encoder_conds[i]
                                        )

                        # sample noise, call unet, get target
                        noise_pred, target, timesteps, huber_c, weighting = (
                        ...
                        loss = self.post_process_loss(
                            loss, args, timesteps, noise_scheduler
                        )
                    ...
                    live.log_metric("loss", current_loss)
                    ...
                    if len(accelerator.trackers) > 0:
                        ...
                        for i, lr in enumerate(lr_scheduler.get_last_lr()):
                            live.log_metric(f"learning_rate/lr_group_{i}", lr)
                        ...
                        if grad_norms:  # Check if there are any gradients
                            max_grad_norm = max(grad_norms)
                            live.log_metric("gradient/max_norm", max_grad_norm)
                        else:
                            live.log_metric("gradient/max_norm", 0)
                ...
                if args.save_every_n_epochs is not None:
                    ...
                    if is_main_process and saving:
                        ...
                        remove_epoch_no = train_util.get_remove_epoch_no(
                            args, epoch + 1
                        )
            ...
            if is_main_process:
                ckpt_name = train_util.get_last_ckpt_name(
