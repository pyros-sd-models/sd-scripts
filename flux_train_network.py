...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    def assert_extra_args(self, args, train_dataset_group):
        ...
        if args.max_token_length is not None:
            logger.warning(
                "max_token_length is not used in Flux training / max_token_lengthはFluxのトレーニングでは使用されません"
            )
        ...
        if args.cache_text_encoder_outputs:
            ...
            with accelerator.autocast():
                dataset.new_cache_text_encoder_outputs(
                    text_encoders, accelerator.is_main_process
                )
    ...
    def sample_images(
        ...
        class FluxUpperLowerWrapper(torch.nn.Module):
            def __init__(
                self,
                flux_upper: flux_models.FluxUpper,
                flux_lower: flux_models.FluxLower,
                device: torch.device,
            ):
                super().__init__()
                self.flux_upper = flux_upper
    ...
    def get_noise_pred_and_target(
        ...
        if not args.split_mode:
            ...
            assert (
                network.train_blocks == "single"
            ), "train_blocks must be single for split mode"
            with accelerator.autocast():
